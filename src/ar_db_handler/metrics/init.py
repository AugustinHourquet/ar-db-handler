...
def _read_schema() -> str:
    return files("ar_db_handler.metrics").joinpath("schema.sql").read_text()


def init_metrics_db(path: str | Path) -> sqlite3.Connection:
    """Initialise metrics.db and return an open connection.
    ...
    logger.info("Initialised metrics.db at %s", path)
    return conn
